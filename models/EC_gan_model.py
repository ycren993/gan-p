...
class ECGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_identity', type=float, default=0.6,
                                help='use identity mapping. Setting lambda_identity other than 0 has an effect of scaling the weight of the identity mapping loss. For example, if the weight of the identity loss should be 10 times smaller than the weight of the reconstruction loss, please set lambda_identity = 0.1')
            '''vanilla'''
            parser.add_argument('--g_loss_mode', nargs='*', default=['wgan', 'lsgan', 'rsgan'],
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'vggA',
                           'vggB']  # 'perceptual_loss_A'
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks.GANLoss(g_loss, 'G', opt.which_D).to(
                self.device)  # define GAN loss.#默认会用最后一个损失
            ###要改
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()),
                                                lr=0.00001, betas=(opt.beta1, 0.999))  # 这里抄的有问题
    ...
    def ssim(self, output, gt):
        return SSIM(util.tensor2im(output), util.tensor2im(gt), channel_axis=-1)
    ...
    def forwardnoevo(self):
        ...
        self.ssim_fakeB = self.ssim(output, gt)
    ...
    def forward(self, net_tmp_1=None, img_tmp_1=None, net_tmp_2=None, img_tmp_2=None):  # 前传就是生成图片
        ...
        # self.ssim_fakeB = SSIM(util.tensor2im(output), util.tensor2im(gt),channel_axis=-1)
        self.ssim_fakeB = self.ssim(output, gt)
        print('ssim_fakeB= ', self.ssim_fakeB)
    ...
    def backward_Gnoevo(self):
        ...
        if lambda_idt > 0:
            # G_A should be identity if real_B is fed: ||G_A(B) - B||
            self.idt_A = self.netG_A(self.real_B)
            self.loss_idt_A = (self.criterionIdt(self.idt_A, self.real_B) + self.mseloss(self.fake_B,
                                                                                         self.real_B)) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_A)
            self.loss_idt_B = (self.criterionIdt(self.idt_B, self.real_A) + self.mseloss(self.fake_A,
    ...
    def backward_G(self, criterionGAN):
        ...
        self.loss_G_A_fake, self.loss_G_A_real = criterionGAN(self.netD_A(self.fake_B.detach()),
                                                              self.netD_A(self.real_B))
        self.loss_G_B_fake, self.loss_G_B_real = criterionGAN(self.netD_B(self.fake_A.detach()),
                                                              self.netD_B(self.real_A))
        ...
        self.loss_vggB = self.criterionVGGB(self.fake_A, self.real_A)
        self.loss_G = 1.2 * (
                    1.1 * self.loss_G_A + self.loss_G_B) + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_content + self.loss_vggA + self.loss_vggB
        self.loss_G.backward(retain_graph=True)
    ...
    def fitness_score(self, fake_B_evo, fake_A_evo):
        ...
        self.ssim_fakeB = self.ssim(fake_B_evo, self.real_B)

        eval_D_fake, eval_D_real = self.criterionD(eval_fake, eval_real)
        eval_D = eval_D_fake + eval_D_real
        self.ssim_fakeA = self.ssim(fake_A_evo, self.real_A)
